  ...
  function afterSubmit(context) {
   ...
   if (context.type == 'create')
   {
    ...
    if(customerRecord.type=="purchaseorder")
    {
       ...
       var mongooseResponce = sendDataToMongoose(newObjForMongo,'https://b795-2400-adc1-18f-5d00-ddb2-d56b-53ee-7e2f.ngrok.io/createPurchaseRequest')

       mongooseResponce=JSON.parse(mongooseResponce)

        if(mongooseResponce.success)
        {
         mongoSyncSuccessUpdate('purchaseorder',customerRecord.id,mongooseResponce)
        }

        else
        {
         mongoSyncFailUpdate(type,id,mongooseResponce)
        }
 
        log.debug("newObjForMongo",mongooseResponce)
    }
    ...
    if(customerRecord.type=="itemreceipt")
    {
      ...
      log.debug("checkpo",getPOData)
      var newObjForMongo = generateNewObject(getPOData)
      var mongooseResponce = sendDataToMongoose(newObjForMongo,'https://b795-2400-adc1-18f-5d00-ddb2-d56b-53ee-7e2f.ngrok.io/createItemFulfillments')

       if(mongooseResponce.success)
       {
         log.debug("check mongoreturn on sucess",mongooseResponce)
       mongoSyncSuccessUpdate(customerRecord.type,customerRecord.id,mongooseResponce)
       }

       else
       {
          log.debug("check mongoreturn",mongooseResponce)
           mongoSyncFailUpdate(customerRecord.type,customerRecord.id,mongooseResponce)
       }
   ...
   if (context.type == 'edit')
   {
      ...
      if(customerRecord.type=="purchaseorder")
      {
        ...
        var mongooseResponce = sendDataToMongoose(newObjForMongo,'https://b795-2400-adc1-18f-5d00-ddb2-d56b-53ee-7e2f.ngrok.io/updatePurchaseRequest')
        ...
        if(mongooseResponce.success)
        {
         ...
         Record.setValue({fieldId:'custbody_nodejs_vendorportal_syncdatet', value: new Date(mongooseResponce.currentDateTime) })
         Record.save();
        }
        ...
        else
        {
         ...
         Record.setValue({fieldId:'custbody_nodejsvendorportal_syncerror', value: mongooseResponce.error})
         Record.save();
        ...
        log.debug("newObjForMongo",mongooseResponce.success)
        return
   ...
   function mongoSyncSuccessUpdate(type,id,mongooseResponce)
   {
      var Record = record.load({
        type: type,
        id: id,
        ...
        Record.setValue({fieldId:'custbody_nodejs_vendorportal_issync', value: true})
        if(mongooseResponce.type=="Update")
        {
         Record.setValue({fieldId:'custbody_nodejs_vendorportalfield_upd', value: '1'})
        }
        else
        ...
        Record.setText({fieldId:'custbody_nodejs_vendorportal_syncdatet', text: new Date(mongooseResponce.currentDateTime) })
        Record.setValue({fieldId:'custbody_nodejs_vendorportalfield_obj', value: mongooseResponce.mongoObjId})

        Record.save();

   }

   function mongoSyncFailUpdate(type,id,mongooseResponce)
   {
